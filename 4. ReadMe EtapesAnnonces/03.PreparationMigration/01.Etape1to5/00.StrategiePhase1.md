...
---
## **Nous travaillons actuellement sur la Migration Phase 1** **-> Etape 1 à Etape 5**
...
**Ce qui reste en localStorage dans cette phase 1** : 
    - Les **7 générations effectuées en retour par OpenAI** restent en l'état, on les laisse temporairement dans `localStorage`
...
## **7 Directives préparatoires impératives**
- Pour ne commettre aucune erreur dans la préparation de la migration.
- **7 Documents d'Etudes Obligatoires sont créés et utilisés**
    ...
    - **2/ La préparation de la table** **-> FAIT**
        - public/4. ReadMe EtapesAnnonces/03.PreparationMigration/01.Etape1to5/01.Table_etapes1to5.md
          - Une table qui respecte la stratégie multi tenant attendue :
              - public/4. ReadMe EtapesAnnonces/03.PreparationMigration/01.Etape1to5/02.1.GestionStrategieMultiTenant.md
          - La fonction timer sera laissée en stand by et ne sera pas intégrée dans la migration
              - public/4. ReadMe EtapesAnnonces/03.PreparationMigration/01.Etape1to5/02.2.FonctionTimer.md
    
    - **3/ Une bible recenssant tous les fichiers, codes, hooks, localstorage utilisés actuellement**  **-> FAIT**
    ...
    - **5/ Un document préparatoire en cours d'élaboration avec identifications des fichiers, hooks, sql... à créer, avec le cahier des charges précis à intégrer**  **-> EN COURS D ANALYSE**
    ...
    - **6/ Un document préparatoire avec tous les codes complets** **-> A FAIRE**

    - **7/ La migration ne pourra être lancée avant la validation du document 6**
---


## **IMPÉRATIF ARCHITECTURE : CONSERVATION DU NOMMAGE `camelCase`**

### **Contexte historique**
- Le `localStorage` actuel de l'application utilise une **nomenclature `camelCase`** pour tous les champs métier (ex: `agencyName`, `propertyType`, `saleType`, `rentAmount`, `keyElements`, etc.).
- Cette convention a été définie lors de la phase MVP initiale et est **profondément ancrée** dans toute l'architecture applicative.
...
**💥 Pourquoi est-il interdit de transformer? **
1. **Raison n°1** :
   - Les **7 prompts OpenAI** (dans `src/services/openai/1.GenerateurAnnoncesOutilsSeo/7.PromptsOpenAi/`) utilisent des placeholders basés sur la structure `camelCase` actuelle (ex: `[nom de l'agence immobilière]` → `data.agencyName`).
...
2. **Raison n°2** :
   - Les hooks de récupération Phase 2 (`useRecuperationDataReseaux`, `useRecuperationDataSMS`, `useRecuperationAnnonceVersLandingPage`, etc.) s'attendent à retrouver les données `localStorage` en `camelCase`.
   - Le template newsletter (`TemplateNewsletterHTML.ts`) utilise directement `parsedData.agencyName`, `parsedData.propertyType`, etc.
   - **Tout changement nécessiterait une refonte complète de la Phase 2**.

3. **💥 Raison n°3** :
   - Introduction de fonctions de transformation `camelCase ↔ snake_case` augmenterait la complexité, les risques d'erreurs, et la dette technique.
   - Chaque `INSERT/SELECT` nécessiterait une conversion bidirectionnelle → **source de bugs insidieux**.
...
### 🎯 **PostgreSQL supporte nativement le `camelCase`**
- Contrairement à une idée reçue, **PostgreSQL/Supabase accepte parfaitement les noms de colonnes en `camelCase`**
- (ils sont simplement encadrés par des guillemets doubles dans les requêtes si nécessaire).
- Il n'y a donc **aucune contrainte technique** empêchant cette approche.
...
- L'objectif est de reprendre le travail sur l'application
  - Lors du travail préparatoire MVP, nous avons travaillé en localstorage pour paramétrer mes api avec Open ai et préparer tous les codes
...
### Mon Objectif global
 - Créer une architecture SaaS multi-tenant complète permettant
   ...
   - Concrètement l'objectif est de mettre en place la structre avec Supabse pour que lorsqu'un utilisateur se connecte (reseau, reseau direction, agence independante etc)
...
- Chaque entité dispose d’un espace de travail isolé et sécurisé
