...
```
```typescript
# **ETAPE 5 : PREPARER LA PHASE 2**
---
- Nous devons préparer la phase 2
- Je te rappelle que les process sont ici
- public/4. ReadMe EtapesAnnonces/02.Etape6Communication
- Lis ce document préparatoire :
- public/4. ReadMe EtapesAnnonces/03.PreparationMigration/02.Etape6communication/01--1.AuditPhase2.md
- Nous allons d'abord analyser comment sont récupérer les données de Openai
- Nous allons devoir savoir comment elles sont anlysés par les différents canaux
- Nous n'allons pas travailler sur tous les canaux, nous allons travailler 1 par 1
- Ta première mission va consister à faire un audit précis des process de récupération des données de Openai
- Comment elles sont injectées dans les canaux
- je ne veux pas de codes ni de stratégie, je veux juste une présentation claire sous form de md
- OK?
```
```typescript
