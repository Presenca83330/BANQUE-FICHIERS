...
# 📌 FormReseauGestion – Projet d'implantation 
Ce document centralise **tous les fichiers** liés au formulaire de gestion réseau.  
Il est conçu pour être intégré directement dans **Lovable** par copier/coller.
Voici la version corrigé du mercredi 24 septembre

---

### Emplacements futurs impératifs
...
ANCIENNE VERSION - INTERDICTION DE MODIFIER - LAISSER EN L ETAT - CORRIGER DESSOUS 
```typescript
...
ANCIENNE VERSION - INTERDICTION DE MODFIER LAISSER EN L ETAT
```typescript
// 📄 src/components/HOOKS-STRATEGIQUE/6.HOOKS-GestionCompteAdminPresenca/1.Reseau/hooks/useReseauIntegrations.ts
...
export function useReseauIntegrations() {
  ...
  const loadForReseau = useCallback(async (reseauId: string) => {
    ...
    try {
      // Charger le réseau via direct supabase car on a besoin de colonnes spécifiques
      const { data: reseau, error } = await supabase
  ...
  // ================================
  // Sauvegarde d'une intégration (factorisé)
  // ================================
  const saveIntegration = useCallback(
    async (reseauId: string, kind: 'brevo' | 'zoho' | 'openai') => {
      ...
      try {
        let tableName, state, setStateFn, fkField, idField;
        ...
        if (connexionId) {
          // Update existante
          const result = await supaOps.update(tableName, idField, connexionId as string, state);
          ...
          const payload = {
            ...state,
            reseau_id: reseauId,
            // organisation_id sera automatiquement ajouté par useSupabaseOperations
          };
          ...
          if (insertedId) {
            // Mettre à jour la FK dans reseau via direct supabase
            await supabase
              .from(getTableName('reseau'))
    ...
    [organisationId, brevo, zoho, openai, supaOps, toast]
  );
...
Coorection de la page suite à modifs Types et useReseauIntegration
Ancien code 
```
 const {
    reseaux,
...
const {
  ...
  zoho, setZoho,
  openai, setOpenAI,
  // 🆕 Ajouter les nouveaux IDs techniques
  brevoConnexionId,
  zohoConnexionId,
  openaiConnexionId,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ⚙️ Si logo fourni → forcer stockage dans dossier 1-logos/
    ...
    // ⚙️ Si documents fournis → forcer stockage dans dossier 2-documents-institutionnels/
    ...
    //    Copie forcée depuis reseau_telephone / reseau_email
    // ============================
...
--   reseau_logo        → chemin unique (string) stocké dans reseau.reseau_logo
--   reseau_ressources  → array de chemins stocké dans reseau.reseau_ressources
--
-- Exemple stockage :
--   reseau_logo :
...
--   -> Chaque utilisateur n'accède qu'à son dossier organisation-{uuid}
--   -> Admin Presenca dispose d'un accès global
--   -> Cohérence avec l'architecture multi-tenant existante (get_user_organisation_id)


```
---
NOUVELLE VERSION CORRIGEE - POLNAME
```sql
-- =========================================
-- 1. Création du bucket unique multi-tenant
-- =========================================
...
| ------------ | -------------------------------------------- | ----------------------------------------------------------------------------------------------- | --------------------- |
| Général      | `useReseauFormData`, `FormReseauGestion`     | `reseau` (point de vérité email/téléphone), `reseau_direction` (héritage)                       | `update-reseau`       |
| Intégrations | `useReseauIntegrations`, `FormReseauGestion` | `reseau` (FKs vers connexions), `brevo_connexion`, `zoho_connexion`, `openai_connexion`         | `update-reseau`       |
...
```
## 9. Rappel des emplacements impératifs
- src/components/HOOKS-STRATEGIQUE/6.HOOKS-GestionCompteAdminPresenca/1.Reseau/hooks/types.ts
- src/components/HOOKS-STRATEGIQUE/6.HOOKS-GestionCompteAdminPresenca/1.Reseau/hooks/useReseauFormData.ts
- src/components/HOOKS-STRATEGIQUE/6.HOOKS-GestionCompteAdminPresenca/1.Reseau/hooks/useReseauIntegrations.ts
