# 📌 FormReseauGestion – Projet D'implantation Corrective

Ce document centralise **tous les fichiers corrigés** liés au formulaire de gestion réseau.  
Il est conçu pour être intégré directement dans **Lovable** par copier/coller.
Voici la version corrigé vendredi 19 septembre -à partir de 15h
...
export interface ReseauFormData {
  ...
  reseau_ressources?: string[] | null;   // ✅ remplacé reseau_documents
...
export interface BrevoIntegration {
  ...
  client_id: string;
  client_type: 'reseau';
  brevo_api_key?: string | null;           // ✅ devenu nullable
  brevo_email_compte?: string | null;     // ✅ devenu nullable
  brevo_nom_compte?: string | null;       // ✅ devenu nullable
}
...
export interface ZohoIntegration {
  ...
  client_id: string;
  client_type: 'reseau';
  zoho_api_key?: string | null;           // ✅ devenu nullable
  zoho_email_compte?: string | null;      // ✅ devenu nullable
  zoho_nom_compte?: string | null;        // ✅ devenu nullable
...
export interface OpenAIIntegration {
  ...
  client_id: string;
  client_type: 'reseau';
  openai_api_key?: string | null;         // ✅ devenu nullable
  openai_email_compte?: string | null;    // ✅ devenu nullable
  openai_project_id?: string | null;
  openai_organisation_id?: string | null;
}
...
export function useReseauFormData() {
  ...
  /**
   * Charger la liste des réseaux disponibles (sélecteur dropdown)
   */
      ...
      try {
        const { data: reseau, error } = await supabase
          ...
          .select(
            ...
            reseau_statut,
            reseau_logo,
            reseau_ressources, -- ✅ corrigé : plus reseau_documents
          ...
          .maybeSingle(); // ✅ robuste si 0 résultat
        ...
        const direction_telephone =
          reseau.reseau_direction?.[0]?.reseau_direction_telephone ?? null;
        const direction_email =
          reseau.reseau_direction?.[0]?.reseau_direction_email ?? null;
  ...
  /**
   * Sauvegarder les données d’un réseau (appel Edge Function update-reseau)
   */
...
Ce document est déjà en place
src/components/ADMIN-PRESENCA/9-CreationComptesUtilisateurs/3-Utilitaires/tabs.tsx
Il permet de créer les onglets de couleur
```
...
const FormReseauGestion: React.FC<Props> = ({ onBack }) => {
  ...
  const uploadFile = async (type: 'logo' | 'document', file: File, reseauId: string) => {
    ...
    if (error) throw error;
    // ↳ l’edge function corrigée renvoie { ok, path, bucket, mime, size }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ✅ Mise à jour dans la table reseau
    const { error } = await supabase
      .from('reseau')
      .update({
        reseau_nom: generalData.reseau_nom,
        reseau_identite_commerciale: generalData.reseau_identite_commerciale,
        reseau_adresse: generalData.reseau_adresse,
        reseau_code_postal: generalData.reseau_code_postal,
        reseau_ville: generalData.reseau_ville,
        reseau_siret: generalData.reseau_siret,
        reseau_statut: generalData.reseau_statut,
        reseau_logo: generalData.reseau_logo ?? null,
        reseau_ressources: generalData.reseau_ressources ?? [], // ✅ corrigé : pas reseau_documents
        ...
        reseau_openai_connexion_id:
          generalData.reseau_openai_connexion_id ?? null,
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM pg_policies
    WHERE polname = 'RLS reseau files access'
  ) THEN
...
-- Chaque utilisateur authentifié ne peut voir/écrire
-- que dans son propre dossier reseau-{reseau_id}
-- =========================================
...
USING (
  ...
  AND (
    -- Admin Presenca a toujours accès
    is_admin_presenca(auth.uid())
    OR
    -- Utilisateurs liés à un réseau ont accès uniquement à leur sous-dossier
    (
      position(('reseau-' || get_user_organisation_id(auth.uid())::text) in name) = 1
    )
  )
);
...
|--------------|----------------------------------|----------------------------------------------|------------------------|
| Général      | useReseauFormData, FormGestion   | reseau, reseau_direction (lecture seule)     | update-reseau          |
| Intégrations | useReseauIntegrations, FormGestion | reseau, brevo_connexion, zoho_connexion, openai_connexion | update-reseau          |
...
└─ components/
├─ HOOKS-STRATEGIQUE/
│ └─ 6.HOOKS-GestionCompteAdminPresenca/1.Reseau/
│ ├─ hooks/
│ │ ├─ types.ts
...
│ └─ components/
│ └─ ReseauSelector.tsx
└─ ADMIN-PRESENCA/9-CreationComptesUtilisateurs/1-Formulaires/1.Reseau/
└─ src/components/ADMIN-PRESENCA/9-CreationComptesUtilisateurs/1-Formulaires/1.Reseau/3.FormReseauGestion.tsx

supabase/
└─ functions/
├─ update-reseau/index.ts
└─ upload-reseau-files/index.ts

supabase/migrations/
└─ 20250917_bucket_table_reseau.sql
```
